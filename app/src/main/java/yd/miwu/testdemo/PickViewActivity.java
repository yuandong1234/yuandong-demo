...
import yd.miwu.R;
import yd.miwu.mode.ProvinceBean;

public class PickViewActivity extends Activity {

    private final static String TAG=PickViewActivity.class.getSimpleName();
...
//        pvOptions.setLabels("省", "市", "区");
        ...
        pvOptions.setOnoptionsSelectListener(new OptionsPickerView.OnOptionsSelectListener() {
            ...
            public void onOptionsSelect(int options1, int option2, int options3) {
                //返回的分别是三个级别的选中位置
                String tx = options1Items.get(options1).getPickerViewText() + options2Items.get(options1).get(option2) + options3Items.get(options1).get(option2).get(options3);
                address.setText(tx);
            }
        });
        //点击弹出选项选择器
